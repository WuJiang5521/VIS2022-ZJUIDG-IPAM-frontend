...
export default inject('analysis')(observer(function Axis({analysis}) {
    ...
    const path = `M${r * Math.cos(sr)} ${r * Math.sin(sr)}A${r} ${r} 0 0 1 ${r * Math.cos(tr)} ${r * Math.sin(tr)}`;
    const textPath = `M${textR * Math.cos(sr)} ${textR * Math.sin(sr)}A${textR} ${textR} 0 0 1 ${textR * Math.cos(tr)} ${textR * Math.sin(tr)}`;
    return <svg width={'100%'} height={'100%'} viewBox={'0 0 100 100'}>
        <g transform={'translate(50,50)'}>
            {attrs.map((attr, aId) => {
                const pathId = `proj-textpath-${attr}`;
                return <g transform={`rotate(${aId * 360 / attrs.length})`}>
